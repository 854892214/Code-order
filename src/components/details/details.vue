  ...
  <div class="mui-pages">
    <div id="billDetail" class="mui-page mui-page-center" v-show="isGetData==false">
      <div class="mui-page-content">
        ...
        <!-- 下单后显示的页面 -->
        <div
          class="bill-detail-content"
          v-if="this.$store.getters.GetIsAddOrder==true||this.$store.getters.GetIsAddOrder=='true'"
        >
        ...
        >
          <div class="title order-info">
            ...
            <span
              id="clearOrder"
              style="padding: 3px 10px 2px 10px;position: relative;bottom: 2px;"
              @click="Cancel"
            >取消</span>
          </div>
          <div class="info">
...
export default {
  ...
  methods: {
    ...
    //点击下单按钮
    addGoodsOrder() {
      if (this.uName !== "" && this.uTel !== "" && this.ShopCartList !== "") {
        this.$store.commit("updateIsAddOrder", true);
      } else if (
          ...
          setTimeout(() => {
            ...
            MessageBox.alert("操作成功").then(action => {
              ...
              setTimeout(() => {
                ...
                this.$store.commit("updateEndList", []);
                this.$store.commit("updateEndLists", '');
  ...
  created() {
    //计算总价
    this.CputeSum();
    //console.log(3,this.ShopCartList)
    for (var item of this.ShopCartList) {
        //order传来什么这里就加入什么
      this.EndList.push(item);
    }
    //点菜后直接买单=>点菜后需要加单=>点菜后取消订单
    //this.ShopCartList=>EndList.push(this.ShopCartList)=>MenuList=[],EndList=[]
    ...
    /* this.$store.commit(
      "updateEndLists",this.$store.getters.GetEndList
    ); */
    //console.log(this.EndList)
    //console.log(this.$store.getters.GetEndList)
    //console.log(this.$store.getters.GetEndLists);
  },
  mounted: function() {}
};
</script>
